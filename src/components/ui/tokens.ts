...
export const type = {
   ...
   *
   * Family is Work Sans, not the Sora heading font: this token carries peso
   * amounts (trip expense totals, scrapbook stats) and Sora's font file has
   * no `₱` (U+20B1) glyph at any weight — verified against the shipped TTFs.
   * Work Sans has full ₱/ñ coverage at every weight, so it is used for every
   * numeral-carrying token, not just this one.
   */
  display: {
    fontSize: 24,
  ...
  /**
   * Group headers above inset lists. Sentence case — Direction A drops the
   * letter-spaced all-caps treatment (a Hard Rule violation); this token
   * keeps its name for callers but no longer implies uppercase transform.
   */
  overline: {
    fontSize: 11,
    fontFamily: 'WorkSans-SemiBold',
...
export const semantic = {
  light: {
    // Palm — paid, done, confirmed.
    success: '#256D4F',
    successSurface: '#E7F3ED',
    // Amber — pending, due, needs attention.
    warning: '#C97A2B',
    warningSurface: '#FBF0E1',
    // Signal Red — destructive/overdue only, never decoration.
    danger: '#B23B33',
